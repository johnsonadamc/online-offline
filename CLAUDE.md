...
SinglePhoto, MultiPhoto2Stacked, MultiPhoto2SideBySide, MultiPhoto4Feature, MultiPhoto4Grid, CollabPage
...
magazine_generation_jobs table exists but pipeline writes to /tmp only — no DB job tracking yet
Music QR code generation not yet implemented — MusicPage renders placeholder
volume and issue are hardcoded in generator.ts — should come from periods table
...
PDF output: /tmp/magazine-{curatorId}-{periodId}.pdf
...
Confirm with git log --oneline -3 that commits are on main
...
Magazine template system — 19 active templates (18 + BlankPage)
...
Curator magazine preview — browser preview route not yet built.
...
Magazine generation job tracking — pipeline writes to /tmp but does not record in magazine_generation_jobs table.
Volume/issue dynamic — volume: 'I', issue: 1 hardcoded in generator.ts. Add volume and issue fields to periods table and read dynamically.
